...
Tip: you may find that your editor uses a different highlighting colour for libraries you have imported but not (yet) used, facilitating the task of pruning your code when finalizing your notebook.
...
```python
# First let's create some strings to access.
# The backslash serves only to indicate that the subsequent line of code
# belongs with the first (I am breaking the line to fit it to the page):
...
# Now we produce a dictionary of characters or sequences we want
# replaced in our documents (alternatively, place inside the function).
# Each entry in a dictionary is a pair, referred to as key and value:
substitutions = {
	'ð': 'þ',
	'ę': 'æ'
	}

# The function itself consists of just these four lines.
# You may replace "k,v" with anything you like, these
# are just the conventional variables for "key" and "value":
def normalize(text):
	for k,v in substitutions.items():
		text = text.replace(k,v)
	return text
    
# Now we have access to our new function whenever we need it:
for input in [string1, string2]:
	print(normalize(input))
```

It is good practice when learning a programming language to train yourself to predict (or rather infer) the outcome of a block of code when you see it, same as in algebra. Can you tell what the above code will output?

# Learning Python

The present guide serves to give you some pointers, not to teach Python as such. Because you will be engaging in NLP (natural language processing), a good first introduction to Python is the online textbook [_Natural Language Processing with Python_](https://www.nltk.org/book/), built atop the NLTK library. Beyond that, [Real Python](https://realpython.com/) has an amazing collection of guides on various tasks and aspects of Python, many of which are freely available.
...
`Shift+Enter`	Run cell code and advance
...
`dd`			Delete current or selected cell(s)
--------------	----------------------------------
